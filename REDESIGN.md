...
|---|------|-----|-------------------------------|--------|
| BUG-1 | `game_screen.dart` | If the player pauses in the first ~400ms, `_startSpawnTimer()` makes a spawn timer while paused. On resume, a second timer starts. The first timer never stops, so words spawn twice as fast. | Yes — spawn logic will change. The same risk can come back in the new code. | **No longer applies** |
| BUG-2 | `game_screen.dart` | No automatic pause when the app goes to the background. The `Stopwatch` keeps counting (best time too long). Animations stop, but the spawn timer keeps adding words. | Yes — **D22** makes all timers stop when the grid is not visible. Check this again after the build. | **Fixed** |
| BUG-3 | `game_screen.dart` | The layout uses a card width of 190px, but a card can be 210px wide. Wide cards at the right edge lose up to 20px. A clue on two lines makes a card taller than the 70px that the overlap check uses. | Yes — card placement will change. | **No longer applies** |
| BUG-4 | `assets/data/word_bank.json` | Two hints do not match their words: `PROFESSOR` → `P----ES-OR` (letter 6 must be `S`); `PLAYGROUND` → `PLA---R-U-` (letter 7 must be `O`). | No | **Fixed** |
| BUG-5 | `pubspec.yaml` | `assets/audio/` is in the asset list, but Git does not store the empty folder. A fresh clone fails `flutter test` and builds. | No | **Fixed** |
| BUG-6 | `test/widget_test.dart` | The smoke test fails: the splash screen's 1500ms `Future.delayed` is still pending when the test ends. | No | **Fixed** |
| BUG-7 | `game_manager.dart` | Unused duplicate logic: `_score`, `_lives`, `checkAnswer()`, `_handleCorrectAnswer()`, `_handleWrongAnswer()`, `_advanceToNextLevel()`. The game screen does not use them. | Maybe — `GameManager` will change. | **Fixed** |
| BUG-8 | `main.dart` | Wrong comments: the splash screen does not load progress (Level Select does), and the orientation comment says upside-down portrait is blocked (the code only sets `portraitUp`). | No | **Fixed** |
| BUG-9 | `PROGRESS.md` | Names a method `advanceLevel()` that does not exist. | Yes — `PROGRESS.md` will be rewritten. | **Fixed** |
| BUG-10 | `game_manager.dart` | 2 analyzer info notes: unnecessary string interpolation at lines 641 and 658. | Maybe | **Fixed** |
| BUG-15 | `game_screen.dart` | **Found and fixed inside stage 4.5, 2026-09-21.** The arrows needed a `Stack` over the grid. A plain child of a `Stack` gets **loose** constraints, so the `SingleChildScrollView` sized itself to its content (296px) instead of the screen area, and the grid stopped scrolling: the lower rows were simply cut off. **Fixed** with `Positioned.fill` around the scroll view. Lesson: a scroll view inside a `Stack` always needs `Positioned.fill`. | Yes — found by the redesign | **Fixed** |
| BUG-14 | `main_menu_screen.dart` | **Found on the phone 2026-09-21** while testing at 720 × 1150. The main menu shows "BOTTOM OVERFLOWED BY 3.2 PIXELS". The menu column has no scroll view, so a short screen overflows. It does not happen at the phone's real size. The same risk applies to a phone with large system text. | No — it is older than the redesign | **Fixed** |
...
| BUG-11 | startup (web build) | An uncaught `AssertionError` appears in the browser console during start, **before** the word bank loads. It happens in `main()` or in the engine start, not in the game screen. The app then runs correctly. Found on 2026-09-20 with `flutter run -d web-server` in debug mode. The cause is not identified. The orientation lock in `main.dart` is the first suspect, because a browser on a desktop cannot lock the screen orientation. **Checked on Android on 2026-09-20: the phone log shows no assertion and no exception, so this is a web-only problem.** | No | **Fixed** |

### Step 5 result: every bug is closed (2026-09-21)

| # | What happened |
|---|---------------|
| BUG-1 | **No longer applies.** The spawn timer is gone. `_startNewCardTimer()` always cancels the old timer before it starts a new one, and the level start is guarded. `_runCountdown()` also refuses to run while the game is paused, which was the old trap. |
| BUG-2 | **Fixed** by D22: `didChangeAppLifecycleState()` pauses everything when the app leaves the foreground. |
| BUG-3 | **No longer applies.** Cards sit in a fixed grid. There is no free placement and no overlap check. |
| BUG-4 | **Fixed.** `PROFESSOR` → `P---ES-OR` (was 10 characters for a 9-letter word) and `PLAYGROUND` → `PLA--R-U--` (the `R` and the `U` were in the wrong places). A script then checked all 300 hints against their words: every hint now has the right length, shows only real letters in the right positions, and hides at least one letter. |
| BUG-5 | **Fixed.** `assets/audio/.gitkeep` keeps the folder in Git, so a fresh clone builds. The file explains why it exists. |
| BUG-6 | **Fixed.** The test now moves the clock forward in 200ms steps until the main menu appears, so the splash screen finishes its 1500ms wait and leaves no timer behind. Small steps matter: the word bank load is real asynchronous work, and one big jump would skip past it. `flutter test` passes. |
| BUG-7 | **Fixed.** `GameManager` lost `_score`, `_lives`, `score`, `lives`, `isGameOver`, `checkAnswer()`, `_handleCorrectAnswer()`, `_handleWrongAnswer()` and `_advanceToNextLevel()`. It now owns the words only. The game screen owns the score, the lives and the end of a level. |
| BUG-8 | **Fixed.** `main.dart` no longer claims that `portraitDown` is in the list, and `progress_manager.dart` no longer claims that the splash screen loads the progress. |
| BUG-9 | **Fixed.** `PROGRESS.md` lists the real methods of `GameManager`. |
| BUG-10 | **Fixed.** The 2 unnecessary string interpolations are gone. `flutter analyze` reports **no issues at all**. |
| BUG-11 | **Fixed.** The cause was the orientation lock: a browser on a desktop cannot lock the screen. `main.dart` now skips that call when `kIsWeb` is true. The browser console shows no `AssertionError` any more. |
| BUG-12 | **Fixed** in stage 4.3 (the Game Over overlay overflowed by 73px). |
| BUG-13 | **Fixed** in stage 4.4 (the broken characters). |
| BUG-14 | **Fixed.** The main menu measures the title height from the space it really has, never below 140px, and it sits in a scroll view. At 380 × 560 in a browser the overflow bar is gone. |
| BUG-15 | **Fixed** in stage 4.5 (`Positioned.fill` for the scroll view inside the `Stack`). |
